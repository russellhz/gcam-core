...
module_policy_L301.ceilings_floors <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L301.StubTech_All <- bind_rows(
      ...
      distinct(bind_rows(L221.StubTech_en, L221.StubTechCalInput_bioOil), region, supplysector, subsector, stub.technology),
    ...
    if (any(!is.na(A_energy_constraints$GDPIntensity_BaseYear))){
      ...
      L301.GDP <- L201.GDP_Scen %>% filter(scenario == socioeconomics.CORE_GCAM_SCENARIO)
