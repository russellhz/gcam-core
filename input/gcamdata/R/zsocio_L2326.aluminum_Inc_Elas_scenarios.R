...
module_socio_L2326.aluminum_Inc_Elas_scenarios <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L102.pcgdp_thous90USD_GCAM_IC_R_Y <- get_data(all_data, "L102.pcgdp_thous90USD_GCAM_IC_R_Y", strip_attributes = TRUE) %>%
      ungroup() %>%
      rename(pcgdp_90thousUSD = value) %>%
      mutate(year = as.integer(year)) %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID")
    ...
    pcgdp_2015_GCAM3 <- L102.pcgdp_thous90USD_GCAM_IC_R_Y %>%
      filter(year == 2015) %>%
      mutate(pcgdp_90thousUSD_2015 = pcgdp_90thousUSD,year = 2020 ) %>%
    ...
    population_2015_GCAM3 <- L101.Pop_thous_GCAM_IC_R_Y %>%
      filter(year == 2015) %>%
      mutate(population_2015 = population ,year = 2020 ) %>%
    ...
    ####Cal gcam3
    #First calculate the per capita aluminum consumption
    L2326.pcgdp_thous90USD_GCAM3_R_Y  <- L102.gdp_mil90usd_GCAM_IC_R_Y%>%
      filter(year %in%  MODEL_FUTURE_YEARS) %>%
      left_join_error_no_match(L101.Pop_thous_GCAM_IC_R_Y, by = c("GCAM_region_ID", "year", "region")) %>%
      left_join_error_no_match(A326.inc_elas_parameter, by = c( "region")) %>%
      mutate(per_capita_aluminum = a * exp(b/(pcgdp_90thousUSD * 1000 * COV_1990USD_2005USD)) * (1-m) ^ (year- 2015),
             aluminum_pro = per_capita_aluminum * population*0.000001)
    ...
    L2326.aluminum_incelas_gcam3 %>%
      ...
      add_precursors("common/GCAM_region_names", "socioeconomics/A326.inc_elas",
                     "L101.Pop_thous_GCAM_IC_R_Y", "L102.gdp_mil90usd_GCAM_IC_R_Y",
                     "L1326.out_Mt_R_aluminum_Yh","L102.pcgdp_thous90USD_GCAM_IC_R_Y") ->
      L2326.aluminum_incelas_gcam3
